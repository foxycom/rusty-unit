...
public class AllMethodTestCaseGenerator implements ChromosomeGenerator<TestCase> {
  ...
  public TestCase get() {
    ...
    int tries = 0;
    while (testCase.size() <= Constants.INITIAL_CHROMOSOME_LENGTH && tries < Constants.INITIAL_CHROMOSOME_LENGTH) {
      testCase.insertCallable(callable, testCase.getFilePathBinding().orElse(null));
      tries++;
    }

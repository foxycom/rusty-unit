...
public class SeededTestCaseGenerator implements ChromosomeGenerator<TestCase> {
  ...
  public TestCase get() {
    ...
    testCase.insertCallable(callable, null);
    ...
    while (testCase.size() < Constants.INITIAL_CHROMOSOME_LENGTH) {
      ...
      if (interestingCallables.isEmpty()) {
        testCase.insertCallable(Rnd.choice(hir.getCallables(true)), testCase.getFilePathBinding().orElse(null));
      } else {
        var interestingCallable = Rnd.choice(interestingCallables);
        testCase.insertCallable(interestingCallable, testCase.getFilePathBinding().orElse(null));
      }
    }
    return testCase;
  }

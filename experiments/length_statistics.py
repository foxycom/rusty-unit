...
with psycopg2.connect("dbname=rustyunit user=rust password=Lz780231Ray") as conn:
    ...
    for crate in random_data["crate"].unique():
        rs_values = random_data[random_data["crate"] == crate]["tests_length"]
        seeded_dynamosa_values = seeded_dynamosa_data[seeded_dynamosa_data["crate"] == crate]["tests_length"]
        dynamosa_values = dynamosa_data[dynamosa_data["crate"] == crate]["tests_length"]
    ...
    print(f"RS average: {random_data['tests_length'].mean()}")
    print(f"Seeded DynaMOSA average: {seeded_dynamosa_data['tests_length'].mean()}")
    print(f"DynaMOSA average: {dynamosa_data['tests_length'].mean()}")

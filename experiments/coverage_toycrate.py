...
with psycopg2.connect("dbname=rustyunit user=rust password=Lz780231Ray") as conn:
    ...
    seeded_dynamosa_values = seeded_dynamosa_data['mir_coverage']
    dynamosa_values = dynamosa_data['mir_coverage']
    rs_values = random_data['mir_coverage']

    seeded_mw = mannwhitneyu(seeded_dynamosa_values, rs_values)
    mw = mannwhitneyu(dynamosa_values, rs_values)

    print(f" Seeded DynaMOSA is better in {a12(seeded_dynamosa_values, rs_values) * 100}% of cases")
    print(f"DynaMOSA is better in {a12(dynamosa_values, rs_values) * 100}%")

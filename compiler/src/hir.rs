...
use crate::util::{def_id_to_t, def_id_to_enum, fn_ret_ty_to_t, generics_to_ts, impl_to_def_id, item_to_name, node_to_name, span_to_path, ty_to_param, ty_to_t, is_local, res_to_name, path_to_name, ty_to_name};
...
fn analyze_enum(
    ...
    let generics = generics_to_ts(g, None, tcx);
...
fn analyze_struct(
    ...
    let struct_generics = generics_to_ts(g, None, tcx);
    ...
    match vd {
        VariantData::Struct(fields, _) => {
            ...
            for field in fields.iter() {
                ...
                let param =
                    ty_to_param(Some(&name), field.ty, Some(&self_ty), None, &struct_generics, tcx);
    ...
    for item in items {
        ...
        match &impl_item.kind {
            ImplItemKind::Fn(sig, body_id) => {
                ...
                let fn_generics = generics_to_ts(&impl_item.generics, Some(&associated_types), tcx);

                let mut overall_generics = impl_generics.clone();
                overall_generics.append(&mut fn_generics.clone());

                info!("Returns: {:?}", &sig.decl.output);
                let return_type = fn_ret_ty_to_t(&sig.decl.output, Some(&self_ty), Some(&associated_types), &overall_generics, tcx);
                ...
                for input in sig.decl.inputs.iter() {
                    let param =
                        ty_to_param(None, input, Some(&self_ty), Some(&associated_types), &overall_generics, tcx);
                    if let Some(param) = param {

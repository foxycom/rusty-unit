...
\begin{enumerate}[start=1, label={\bfseries RQ\arabic*:}]
    ...
    \item How many coverage goals does \textsc{RustyUnit} cover?
    % Compare coverage and test case length
    \item How does the \ac{GA} of \textsc{RustyUnit} compare to random search?
    \item Does \textsc{RustyUnit} import coverage of manually written tests in case study subjects?
    \item TODO: compare \textsc{RustyUnit} to the API call tool, whatever it is called.
...
% RQ 1 answer
\begin{tcolorbox}
\textbf{Summary (RQ1):} \textsc{RustyUnit} covers achieves at least as high coverage as random test generation depending on the crate. Moreover, \textsc{RustyUnit} generates shorter tests than random test generation. 
\end{tcolorbox}

...
The main weakness of \ac{MOSA} is that it considers all objectives as equal and independent. Structural programs, however, are fraught with dependencies. For example, some objectives can only be satisfied if another is already covered~\cite{Fraser_2013}. To better explain this concept, let us consider the example in \Cref{lst:dependent-objectives-code}. The four branches to be covered, \textit{b1}, \textit{b2}, \textit{b3}, and \textit{b4}, are not independent from each other. In fact, coverage of \textit{b2}, \textit{b3} can be achieved only after \textit{b1} has been covered since the former targets are under control of the latter. In other words, there is a control dependency between \textit{b1} and \textit{b2},\textit{b3}, which means that the execution of \textit{b2},\textit{b3} depends on the outcome of the condition at node $2$, which in turn is evaluated only once target \textit{b1} is covered. 

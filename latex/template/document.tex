...
Consider, as an example, how heap memory is handled in C and Java. In C we can allocate memory on the heap by using the malloc or calloc functions. The data on the heap can then be reclaimed with free. These functions are easily misused, e.g., by calling free twice on the same pointer, which is undefined behaviour, and forgetting to free memory would be wasting that memory. Java, and many other high level programming languages, use a different approach. Here the programmer does not have to manually allocate and deallocate heap memory, but it is reclaimed at runtime using a garbage collector. While this makes it easier to write programs that correctly handle memory, it also impacts program performance, due to the operational overhead of determining which memory is no longer in use.
...
\begin{description}
  \item[Insert a statement] Ein gegebener Test Case $t$ wird mit einer Wahrscheinlichkeit $\sigma$ um ein neues Statement erweitert, welches an einer zufälligen Position in $t$ eingefügt wird. Ein neues Statement kann nur dann eingesetzt werden, wenn $l(t) < L$, also wenn die Testlänge nicht ein definiertes Maximum überschreitet. For each insertion, with probability $1/3$ a random call out of the pool of callables is inserted, with probability $1/3$ a method call on a value in the set $\{v(s_k) \left| 0 \leq k < i \right\}$ is used as a parameter in a random call. Any parameters of the selected call are either reused out of the set $\{v(s_k) \left| 0 \leq k < i \right\}$ or randomly generated. Jeder Parameter~$p$, der aus $\{v(s_k) \left| 0 \leq k < i \right\}$ ausgewählt wird, muss entsprechend den Regeln aus \Cref{sec:problem-representation} von dem Call verwendbar sein. Die zufällige Position des neuen Statements muss mindestens größer als $max(\{pos_t(a) : a \text{ is an argument of } c\})$ bzw. größer gleich $0$, wenn~$c$ keine Parameter hat. Es können auch mehrere Statements wiederholt eingesetzt werden, wobei dies mit der Wahrscheinlichkeit~$p(n) = 0.5^n$ passiert, wo~$n$ die Anzahl von bisher eingefügten Aufrufen ist. Das heißt, dass nach jedem Einsetzten eine zufällige Entscheidung getroffen wird, ob die Mutation wiederholt werden soll~\cite{Tonella2004}.
...
For the evaluation, we choose ten open source crates from the Cargo's crate registry. This results in a total of 6661 methods.
...
Search algorithms are often compared in terms of the number of fitness evaluations or generated chromosome samples. Das heißt, dass die in der Evaluation verwendeteten Algorithmen das gleiche Budget an generierten Chromosomen haben und die Performanz der Algorithmen wird anhand der Code Coverage von Test Cases verglichen, die übrig bleiben, nach dem die Algorithmen das Budget aufgebraucht haben. Das ist das, was wir für den Vergleich von \textsc{RustyUnit} mit random search heranziehen. As discussed by Ali et al.~\cite{Ali2010}, a search algorithm should always be compared against at least random search in order to check that the algorithm is not simply successful because the search problem is easy. Für die Evaluation haben wir die Option eines random search eingebaut, wodurch zufällige Test Cases auf eine ähnliche Weise generiert werden, wie beim die initiale Population beim genetischen Algorithmus von \textsc{RustyUnit}. Der random search verwendet ebenfalls einen Archiv und speichert Test Cases, die bisher ungedeckte Branches im \ac{SUT} ausführen. Für das random search werden die gleichen Parameter für die maximale Testlänge, Populationsgröße und jegliche Wahrscheinlichkeiten verwendet, wie sie für \textsc{RustyUnit} gesetzt sind. Die Intuition ist, dass der \ac{GA} von \textsc{RustyUnit} mindestens bessere Test Cases in Hinsicht auf die Code Coverage liefert, da die Basis für die Generierung gleich ist, der \ac{GA} von \textsc{RustyUnit} jedoch eine Führung hat.
...
\end{table}
\section{Code Coverage Comparison with KLEE}

...
With the definition from Listing~\ref{lst:static-generics} we can now create pair instances with different types as shown in Listing~\ref{lst:example-generics-usage}, e.g., \lstinline{String} and \lstinline{u32} or two times \lstinline{usize}. The same code is reused and the compiler can statically check whether the values that are used with a pair instance are of the correct type.
\begin{lstlisting}[language=Rust, style=boxed, caption=A function which will remain generic at runtime. It can be rewritten in the same stly as the other two variants in Listing~\ref{lst:example-generic-functions}, label=lst:example-generics-usage]
...
\todo{DefIds und so weiter beschreiben}
...
Hier muss beschrieben werden, wie wir die Assertions in der MIR instrumentieren. Zum Beispiel werden in der MIR bei arithmetischen Operationen Checks eingeführt:

...
We adopt~\Cref{alg:gentest} and~\Cref{alg:genobject} from Fraser and Zeller and use it to generate dependencies of method and function invocations. Since the prototype of \tech does not provide the ability to construct all possible types, e.g., closures or trait objects, \texttt{GenObject} may fail to generate an instance of the appropriate type. In this case, we skip the particular method or function which we tried to generate the argument for. Obviously, this might lead to worse coverage results, but a complete analysis of all possible cases is not trivial and cannot be done within the scope of this work.
...
In this chapter we present essential parts of the implementation of \tech. To generate tests for a crate, \tech has to complete several intermediate steps that \Cref{overview/overview-enhanced} illustrates. First and foremost, the tool requires the information about which data types the \emph{Crate} has and which functions the data types provide in order to be able to model meaningful tests at all. Therefore, it performs a \hir analysis, which yields a collection of \emph{Generators}. \textbf{TODO Generators inkludiert irgendwie eher nur Funktionen und weniger Datentypen an sich}. These provide an overview of which data types can be instantiated in a test case, and which methods can be invoked on those instances. \tech also derives the ownership rules at this point, e.g., whether and how multiple statements may use a certain variable in a valid way, as described in~\Cref{sec:problem-representation}. 
...
\begin{enumerate}
    \item \texttt{config(&mut self, _c: &mut Config)}
    \item \texttt{after\string_parsing(&mut self, _c: &Compiler, _q: &'tcx Queries<'tcx>)}
    \item \texttt{after\string_expansion(&mut self, _c: &Compiler, _q: &'tcx Queries<'tcx>)}
    \item \texttt{after\string_analysis(&mut self, _c: &Compiler, _q: &'tcx Queries<'tcx>)}
\end{enumerate}
...
Within the callbacks, with a compiler and a \texttt{Queries} object given, we can access the \acp{IR} of the crate under compilation, e.g., we can query the \mir of a function body by its \texttt{DefId}, as shown in~\Cref{lst:optimized-mir}. To this end, we retrieve the \texttt{TyCtxt} instance in~Line~\ref{line:enter-tyctxt}, which the central data structure of the compiler and provides methods to query different regions of the compiled code, e.g., \texttt{optimized_mir} in~Line~\ref{line:optimized-mir-call} to obtain the \mir of a function. We cannot modify it and pass the modified version back to the compiler, though, which is critical for \mir instrumentation. This is not a concern for the \hir and \mir analysis, as we only want to read the structure of the crate. For this purpose, we can override the default implementation of the \texttt{optimized_mir} method with a custom one. The \texttt{config} callback provides a mutable \texttt{Config} object which allows to replace many of the query functions. 
  ...
  fn config(&mut self, _config: &mut Config) {
    _config.override_queries = Some(|_, _, external| { §\label{line:mir-provider-start}§
      let fn_ptr: mir_fn_ptr = |tcx, def_id| {
        ...
        if is_monitor(hir_id, &tcx) { §\line{line:is-monitor-check}§
...
\begin{enumerate}
  \item first obtains the original \mir instance in~Line~\ref{line:query-mir} using the default implementation in~Lines~\ref{line:obtain-default-mirprovider-start}-\ref{line:obtain-default-mirprovider-end},
...
\end{lstlisting}
The test in \Cref{lst:building-generic-test-1} is not compilable as is, though. This is why we use \texttt{GenObject} to create an instance of \texttt{Foo}, e.g., by invoking its constructor. The constructor has a parameter of type \texttt{T} which we already have a binding for; it effectively makes us able to call the constructor with a random integer, as shown in~\Cref{lst:building-generic-test-2}.
